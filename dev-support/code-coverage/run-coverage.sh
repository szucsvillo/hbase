...
execute() {
  ...
  mvn -B -e -f "${MAIN_POM}" clean test -PrunAllTests -Pjacoco -Pbuild-with-jdk17 -Dhadoop.profile=3.0 -fn

  echo "Starting verifying phase"

  mvn -B -e -f "${MAIN_POM}" verify -DskipTests -DskipITs -Pjacoco -Pbuild-with-jdk17 -Dhadoop.profile=3.0 -fn
